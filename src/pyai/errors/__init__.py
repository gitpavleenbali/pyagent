...
Provides a structured error hierarchy for pyai.
...
Example:
    from pyai.errors import ModelError, RateLimitError
...
class pyaiError(Exception):
    """Base exception for all pyai errors.

    All pyai exceptions inherit from this class, making it easy
    to catch any pyai-related error.

    Example:
        try:
            agent.run("Hello")
        except pyaiError as e:
            print(f"pyai error: {e}")
    """
...
def is_retriable(error: Exception) -> bool:
    ...
    Example:
        from pyai.errors import is_retriable
