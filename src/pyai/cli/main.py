...
"""
pyai CLI Main Entry Point

Provides CLI commands similar to Google ADK's `adk` CLI:
- pyai run: Run an agent interactively
- pyai eval: Evaluate agent on test cases
- pyai serve: Start API server
- pyai init: Initialize new project
"""
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    Example:
        pyai run my_agent.py
        pyai run config.yaml --input "Hello"
    """
    print(f"🚀 pyai CLI v{__version__}")
...
def cmd_eval(args: argparse.Namespace) -> int:
    ...
    Example:
        pyai eval tests.evalset.json --agent my_agent.py
        pyai eval tests/*.json --parallel
    """
    print(f"🧪 pyai Evaluation CLI v{__version__}")
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    Example:
        pyai serve my_agent.py --port 8000
    """
    print(f"🌐 pyai API Server v{__version__}")
    ...
    try:
        ...
        try:
            ...
            app = FastAPI(
                title="pyai API", version=__version__, description="AI Agent API powered by pyai"
            )
...
def cmd_init(args: argparse.Namespace) -> int:
    """Initialize a new pyai project.

    Example:
        pyai init my-agent
        pyai init . --template research
    """
    print(f"🎉 pyai Project Initializer v{__version__}")
...
if __name__ == "__main__":
    ...
    print(result.output)
''',
            "requirements.txt": "pyai>=0.2.0\n",
            "README.md": f"# {args.name}\n\nAI Agent built with pyai.\n\n## Usage\n\n```bash\npyai run agent.py\n```\n",
        },
        "research": {
            "agent.py": '''"""Research Agent with Tools"""
from pyai import Agent
from pyai.easy import research, fetch, summarize
...
''',
            ...
            "requirements.txt": "pyai>=0.2.0\nrequests\n",
            "README.md": f"# {args.name}\n\nResearch Agent built with pyai.\n",
        },
    }
    ...
    print("   pyai run agent.py")

    return 0


def cmd_info(args: argparse.Namespace) -> int:
    """Show pyai information."""
    print(f"pyai v{__version__}")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="pyai",
        description="pyai CLI - Build and run AI agents",
        epilog="For more info: https://github.com/pyai/pyai",
    )
    parser.add_argument("-V", "--version", action="version", version=f"pyai {__version__}")
    ...
    # Info command
    subparsers.add_parser("info", help="Show pyai info")
