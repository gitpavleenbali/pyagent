"""
LangChain Integration for pyai
==================================

Bridge between pyai and LangChain ecosystems. Import LangChain tools,
chains, and agents into pyai, or export pyai components to LangChain.

Features:
- Import LangChain tools as pyai skills
- Use LangChain chains within pyai workflows
- Export pyai agents as LangChain-compatible
...
Examples:
    ...
    >>> agent = pyai.agent("researcher", tools=[search_tool])
...
@dataclass
class LangChainTool:
    """Wrapper for LangChain tools in pyai."""
    ...
    def to_pyai_skill(self):
        """Convert to pyai skill format."""
        from pyai.easy.mcp import MCPTool
...
def import_tool(tool_name: str, *, api_key: str = None, **kwargs) -> LangChainTool:
    """
    Import a LangChain tool for use in pyai.
...
def import_chain(chain_type: str, *, llm: Any = None, **kwargs) -> Any:
    """
    Import a LangChain chain for use in pyai.

    Args:
        chain_type: Type of chain (e.g., "llm_math", "sql_database")
        llm: LLM to use (uses pyai default if not specified)
...
def export_agent(pyai_agent, *, tools: List = None) -> Any:
    """
    Export a pyai agent as a LangChain agent.
    ...
    try:
        ...
        # Convert pyai to LangChain tool
        lc_tools = []
        if tools:
            for t in tools:
                lc_tools.append(
...
def _get_lc_llm():
    """Get LangChain LLM using pyai configuration."""
    import os
